 ...
 *
 * Arbor deliberately does not infer roadmap stages from tool counts. A roadmap is
...
internal object ResearchStateProtocol {
    const val OPEN_TAG = "<arbor-research-state>"
    const val CLOSE_TAG = "</arbor-research-state>"
    ...
    private val completeBlock = Regex(
        """<arbor-research-state>\s*([\s\S]*?)\s*</arbor-research-state>""",
