...
internal fun SetupRestoreActions(viewModel: ChatViewModel) {
    ...
    fun showBackups(source: SetupCloudSource, values: List<CloudBackupEntry>) {
        cloudSource = source
        entries = values
        error = if (values.isEmpty()) "No Arbor backups were found in this app-only location." else null
    ...
    Surface(
        ...
        Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
            Text("Already use Arbor?", style = MaterialTheme.typography.titleMedium, fontWeight = FontWeight.SemiBold)
            Text(
            ...
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                OutlinedButton(
                    onClick = {
                        localPicker.launch(
                            arrayOf(ARBOR_BACKUP_MIME, ARBOR_CHAT_MIME, "application/octet-stream", "application/zip"),
                        )
                    },
            ...
            text = {
                Column(
                    ...
                    if (cloudSource == SetupCloudSource.CHOOSE) {
                        Text(
                            "Both options use app-only storage. Arbor never asks to browse an entire cloud account.",
                            style = MaterialTheme.typography.bodySmall,
