...
object WidgetPinning {
    ...
    fun request(context: Context, source: String, grants: WidgetCapabilityGrants): WidgetPinResult {
        val definition = ArborProgramParser.parse(source, ArborProgramSurface.WIDGET).getOrNull() ?: return WidgetPinResult.INVALID
        ...
        val preview = ArborHomeWidgetProvider.views(context, AppWidgetManager.INVALID_APPWIDGET_ID, source, grants, preview = true)
...
class WidgetPinReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        ...
        val definition = ArborProgramParser.parse(pending.source, ArborProgramSurface.WIDGET).getOrNull() ?: return
        ...
        AppWidgetManager.getInstance(context).updateAppWidget(id, ArborHomeWidgetProvider.views(context, id, pending.source, pending.grants))
...
class ArborHomeWidgetProvider : AppWidgetProvider() {
    override fun onUpdate(context: Context, manager: AppWidgetManager, appWidgetIds: IntArray) {
        ...
        appWidgetIds.forEach { id ->
            ...
            val definition = source?.let { ArborProgramParser.parse(it, ArborProgramSurface.WIDGET).getOrNull() }
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        val transition = ArborProgramRuntime.apply(actionId, definition, storage.state(id).ifEmpty { definition.state })
...
internal fun grantsSatisfy(definition: ArborProgramDefinition, grants: WidgetCapabilityGrants): Boolean =
...
private const val EXTRA_TOKEN = "arbor_widget_token_v2"
private const val EXTRA_ACTION_ID = "arbor_widget_action_id"
