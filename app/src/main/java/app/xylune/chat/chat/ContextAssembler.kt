...
class ContextAssembler(private val attachmentDao: AttachmentDao) {
    suspend fun assemble(
        ...
        result += InputMessage(
            ...
            If Python needs packages which are not installed, request them in a fenced `python-requirements` block with one package requirement per line. Xylune installs them with pip into `/workspace/.xylune-venv` inside the selected distribution after approval. Xylune asks the user before installing anything; never claim installation until a later system event confirms it.
            ...
            The chat workspace is `/workspace` inside the selected distribution, including `incoming/`. Python and Linux commands run as root (uid 0) inside PRoot. This is a compatibility/tooling layer, not a security boundary; Android still confines the app. Python has a 45-second default deadline and Linux commands have a 60-second default; a request may set `timeoutSeconds`, up to 600 for Python or 900 for Linux. If a result says it timed out, report the exact elapsed time and ask before retrying with a longer deadline—never silently repeat it. Never use apt, dpkg, apk, pip, or another package manager through `linux_exec`. Request packages in a visible fenced `linux-packages` block, one package per line, and wait for Xylune to report the user's configured approval decision and completed installation.
