...
import app.xylune.chat.settings.chromeEdgeCornerTransition
import app.xylune.chat.settings.effectiveChromeEdgeSoftness
import app.xylune.chat.settings.snapChromeEdgeSoftness
...
enum class XyluneBlurEdge { TOP, BOTTOM }

/** Runtime-only developer visualization. Normal chrome never draws boundary lines. */
internal object XyluneBackdropDebugOverlay {
 ...
 */
@Stable
class XyluneBackdropBlurState internal constructor() {
    ...
    internal fun updatePanelBounds(edge: XyluneBlurEdge, startInRootPx: Float, endInRootPx: Float) {
    ...
    internal fun clear(edge: XyluneBlurEdge) {
        when (edge) {
            XyluneBlurEdge.TOP -> {
                topRadiusDp = 0f
...
@Composable
fun rememberXyluneBackdropBlurState(): XyluneBackdropBlurState = remember { XyluneBackdropBlurState() }

/**
 * Applies Xylune's device-proven three-axis edge blur directly to the content
 ...
 */
fun Modifier.xyluneBackdropSource(state: XyluneBackdropBlurState): Modifier = composed {
    ...
    val debugBoundaryEnabled = XyluneBackdropDebugOverlay.enabled
    val debugBoundaryThicknessDp = XyluneBackdropDebugOverlay.thicknessDp
    ...
    val decorated = measured.drawWithContent {
        val started = if (blurActive && XyluneRenderProfiler.enabled) System.nanoTime() else 0L
        drawContent()

        drawPanelOverlay(
            edge = XyluneBlurEdge.TOP,
        ...
        if (blurActive && XyluneRenderProfiler.enabled) {
            XyluneRenderProfiler.recordBlurFrame(
...
internal fun buildPanelEdgeBlurEffect(
    ...
    XyluneRenderProfiler.recordBlurEffectBuild(3)
...
internal fun xyluneBlurProgress(progress: Float): Float {
...
internal fun edgeSoftnessActivation(edgeSoftness: Float): Float =
    xyluneBlurProgress(effectiveChromeEdgeSoftness(edgeSoftness))

/** Registers one panel. The source layer draws both the blur mask and tint. */
fun Modifier.xyluneBackdropBlur(
    state: XyluneBackdropBlurState,
    strength: Float,
    edgeSoftness: Float,
    overlayOpacity: Float = 1f,
    tint: Color,
    edge: XyluneBlurEdge = XyluneBlurEdge.TOP,
    maxRadius: Dp = DEFAULT_MAX_RADIUS_DP.dp,
    panelHeight: Dp = if (edge == XyluneBlurEdge.TOP) DEFAULT_TOP_PANEL_HEIGHT_DP.dp else DEFAULT_BOTTOM_PANEL_HEIGHT_DP.dp,
    ...
    this.onGloballyPositioned { coordinates ->
        ...
        when (edge) {
            XyluneBlurEdge.TOP -> state.updatePanelBounds(edge, bounds.top, bounds.top + effectiveHeightPx)
            XyluneBlurEdge.BOTTOM -> state.updatePanelBounds(edge, bounds.bottom - effectiveHeightPx, bounds.bottom)
        }
    }
}
...
private fun DrawScope.drawPanelOverlay(
    ...
    if (tint.alpha > 0f) {
        if (!softnessActive) {
            ...
            val path = Path().apply {
                when (edge) {
                    XyluneBlurEdge.TOP -> addRoundRect(
                        RoundRect(
                    ...
                    XyluneBlurEdge.BOTTOM -> addRoundRect(
                        RoundRect(
                            0f, start, size.width, end,
                            CornerRadius(radius, radius), CornerRadius(radius, radius),
                            CornerRadius.Zero, CornerRadius.Zero,
    ...
    if (alpha > 0f) {
        val y = if (edge == XyluneBlurEdge.TOP) end else start
        drawLine(
