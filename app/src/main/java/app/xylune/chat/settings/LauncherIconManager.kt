 ...
 * changed, even with DONT_KILL_APP. Xylune therefore saves its complete UI state,
 * dispatches the component mutation to the isolated launcher process, closes the
 * old task deliberately, and lets that process relaunch Xylune from the snapshot.
 */
internal object LauncherIconManager {
    private const val TAG = "XyluneLauncherIcon"
    internal const val EXTRA_DESIRED_ALIAS = "app.xylune.chat.extra.DESIRED_LAUNCHER_ALIAS"
    internal const val EXTRA_RELAUNCH_INTENT = "app.xylune.chat.extra.RELAUNCH_INTENT"

    internal const val XYLUNE_ALIAS = "app.xylune.chat.LauncherXylune"
    internal const val SYSTEM_ALIAS = "app.xylune.chat.LauncherSystem"
    internal const val GRAPHITE_ALIAS = "app.xylune.chat.LauncherGraphite"
    internal const val OCEAN_ALIAS = "app.xylune.chat.LauncherOcean"
    internal const val VIOLET_ALIAS = "app.xylune.chat.LauncherViolet"
    internal const val SUNSET_ALIAS = "app.xylune.chat.LauncherSunset"
    ...
    internal fun aliasClassName(matchPalette: Boolean, palette: ColorPalette): String {
        if (!matchPalette) return XYLUNE_ALIAS
        return when (palette) {
            ColorPalette.XYLUNE -> XYLUNE_ALIAS
    ...
    private fun enabledAlias(context: Context): String? {
        ...
        return allAliases.firstOrNull { className ->
            isEnabled(
                ...
                manifestDefault = className == XYLUNE_ALIAS,
            )
        }
    }
    ...
    private fun applyAtomically(
        ...
        val changes = allAliases.mapNotNull { className ->
            ...
            val manifestDefault = className == XYLUNE_ALIAS
    ...
    private fun applyEnableFirst(
        ...
        setEnabled(
            ...
            enabled = true,
            manifestDefault = desiredClassName == XYLUNE_ALIAS,
        )
