...
class XyluneHapticsIntegrationTest {
    @Test fun highSignalInteractionsUseTheSharedHapticVocabulary() {
        ...
        assertTrue(source("XyluneHaptics.kt").contains("fun streamTick()"))
        assertTrue(source("XyluneHaptics.kt").contains("fun streamComplete()"))
        assertTrue(chat.contains("STREAM_HAPTIC_CHARACTER_INTERVAL = 32"))
    }

    private fun source(name: String) = java.io.File("src/main/java/app/xylune/chat/ui/$name").readText()
}
