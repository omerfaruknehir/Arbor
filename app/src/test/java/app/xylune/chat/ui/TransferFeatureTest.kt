...
class TransferFeatureTest {
    ...
    fun `provider setup selects all discovered models and stores only checked models`() {
        val settings = File("src/main/java/app/xylune/chat/ui/SettingsScreen.kt").readText()
    ...
    fun `portable archives allow explicit unencrypted output and safe chat defaults`() {
        val ui = File("src/main/java/app/xylune/chat/ui/TransferUi.kt").readText()
        val archive = File("src/main/java/app/xylune/chat/transfer/XyluneArchiveManager.kt").readText()
    ...
    fun `incoming archives preview and import as copies`() {
        val ui = File("src/main/java/app/xylune/chat/ui/TransferUi.kt").readText()
        val activity = File("src/main/java/app/xylune/chat/MainActivity.kt").readText()
        ...
        assertTrue(manifest.contains("application/vnd.xylune.chat"))
    }
}
