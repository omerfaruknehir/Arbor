...
class RunRecordStoreTest {
    private val root = Files.createTempDirectory("arbor-run-records").toFile()
    ...
    @Test fun rerunMetadataReferencesCanonicalSourceWithoutDuplicatingIt() {
        ...
        assertEquals(1, File(root, "conversation/.arbor/runs/${metadata.runId}").listFiles { file -> file.name.startsWith("main.") }!!.size)
    }
