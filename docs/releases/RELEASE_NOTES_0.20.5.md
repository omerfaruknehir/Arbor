...
Xylune 0.20.5 corrects the bundled talloc library's license classification and
...
- Explain why the retained historical Termux recipe's package-wide GPL-3.0
  label is broader than the library binary shipped by Xylune.
- Include the LGPL v3 text in both the repository and APK assets.
- Add a dedicated **Third-party notices** entry to About Xylune so the bundled
...
Download `Xylune-0.20.5-debug.apk` from the release assets. It is debug-signed
for direct testing and uses package ID `app.xylune.chat.debug`.
