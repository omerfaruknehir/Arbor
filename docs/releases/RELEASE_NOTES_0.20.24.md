...
- **Google Drive app storage:** authorize the non-sensitive `drive.appdata` scope only. Arbor writes backups to Drive's hidden appDataFolder and never requests My Drive access. Large backups use resumable, chunked uploads with retry and resume handling.
